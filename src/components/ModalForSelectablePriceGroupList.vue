    ...
    <div :class="{'modal':true , 'is-active': true,}" v-if="isActive">
        ...
        <div class="modal-card w-50 h-75">
            <header class="modal-card-head has-text-left">
                <div class="modal-card-title">
                    <div class="columns is-v-centered">
                        <div class="column is-narrow">Groupe tarifaires</div>
            ...
            <section class="modal-card-body" >
                ...
                <div style="margin-left: auto;margin-right: auto;" v-if="filteredPriceGroupList.length == 0 && res_fetching_data_occur">
                    Aucun enregistrement n'a été trouvé
                </div>
                <div v-if="res_fetching_data_occur && filteredPriceGroupList.length > 0 && !error_fetching_data_occur">
                    <table class="table is-narrow is-hoverable is-bordered is-fullwidth shadow tableFixHead">
                        ...
                        <tbody s>
                            <tr  :id="elt['Code']" v-for="(elt,index) of dividedPriceGroupList" :key="index" @mouseover="setLineShadow(elt['Code'])" @mouseout="removeLineShadow(elt['Code'])" @click="$emit('onGettingLineFromSelectablePriceGroupListModal',elt);$emit('closeModal')">
            ...
            <footer class="modal-card-foot">
                <div class="buttons">
                    ...
                    <button class="button has-background-orange has-text-white" @click="pageSize = pageSize + 50" v-if="(pageSize < filteredPriceGroupList.length) && (currentPage !== Math.ceil(filteredPriceGroupList.length/pageSize))">
                        Charger plus
                    </button>
                    <button class="button has-text-orange" @click="nextPage" v-if="currentPage*pageSize < filteredPriceGroupList.length">
                        <span class="icon">
...
export default{
    name:'modal-for-selectable-price-group-list',
    props:['isActive'],
    components:{
        inputSearch
    },
...
//Gestion de la recherche
        ...
        const filteredPriceGroupList = computed(()=>
            elementList.value
...
//Gestion de la pagination
        ...
        const dividedPriceGroupList = computed(()=>{
            if(eltToSearch.value){
                return filteredPriceGroupList.value
                .filter((row, index) => {
        ...
        function nextPage(){
            if((currentPage.value*pageSize.value) < filteredPriceGroupList.value.length) currentPage.value++;
        }
        function prevPage(){
...
//Gestion de la récupération de la liste à afficher
        ...
        function getPriceGroupList(){
            axios.get(`http://${hostname}:5000/app/getPriceGroupList`)
            .then(result => {
        ...
        return {
            eltToSearch,elementList,filteredPriceGroupList,
            dividedPriceGroupList,pageSize,currentPage,nextPage,prevPage,
            getPriceGroupList,
