...
export default {
    ...
    setup(){
        ...
        function getRevisionCardInfo(){
            ...
            axios.get(`http://${hostname}:5000/app/getRevisionRequestCard?documentNo=${revisiondocumentNo.value}`)
            .then(result => {
        ...
        function getHistoryInfo(){
          axios.get(`http://localhost:5000/app/getCUHList?customerId=UDT00001`)
          .then(res =>{
                console.log(res);
      ...
      function getTrackingInfo(){
        axios.get(`http://localhost:5000/app/getApprovalFlow?documentNo==0001`)
        .then(res =>{
            console.log(res);
            if(new Array(res.data[0].length>=0)){
