...
export default {
  ...
  mounted() {
    axios
      .get(
        `http://108.175.0.116:3000/app/getContactCard/${this.contactCardId1}/${this.contactCardId2}`
      )
      .then((result) => {
        console.log("ALLO MALONJU");
        this.ContactCardHeader = result.data.recordset[0];
        this.ContactCardLines = result.data.recordset;
