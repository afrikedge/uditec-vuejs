<template>
    <div class="my-5 mx-5" style="position:fixed;top:150px;bottom: 0px; width:auto;right:6%;left:6%">
        ...
        pageTitle="Demandes validation paiement"
        ...
        <div class="has-background-light columns mb-1 is-gapless" style="overflow-y: hidden;height:auto;position: fixed;bottom:0px;top:200px; width:auto;right:6%;left:6%">
            <div class="column mt-5" id="customer-column" style="overflow: scroll;" v-if="presentationView=='list'">
                <table class="table  is-narrow is-hoverable is-fullwidth tableFixHead">
                    ...
                    <tbody>
                        <tr :id="payment['No_']" v-for="payment of dividedPaymentList" :key="payment['No_']" class="is-narrow"  @mouseover="setLineShadow(payment['No_'])" @mouseout="removeLineShadow(payment['No_'])" @click="selectLine(payment['No_'])">
                            ...
                            <td class="has-text-centered has-background-light is-narrow has-text-weight-bold">
                                <span class="tag is-white box py-0 px-2"  v-if="payment['Approval Status'] == 6">
                                    {{ displayLabel('optionLabelListForApprovalStatus',payment['Approval Status']) }}
                                </span>
                                <span class="tag is-danger is-light box py-0 px-2"  v-if="payment['Approval Status'] == 10">
                                    {{ displayLabel('optionLabelListForApprovalStatus',payment['Approval Status']) }}
                                </span>
                                <span class="tag is-success box py-0 px-2"  v-if="payment['Approval Status'] == 7 || payment['Approval Status'] == 8">
                                    {{ displayLabel('optionLabelListForApprovalStatus',payment['Approval Status']) }}
                                </span>
                            </td>
    ...
    <div style="position: absolute; bottom: 0px; left:0px" v-if="(pageSize < filteredpaymentList.length) && (currentPage !== Math.ceil(filteredpaymentList.length/pageSize))">
    ...
    <div style="position: absolute; top: 50%; bottom: 50%; right:10px" v-if="currentPage*pageSize < filteredpaymentList.length">
...
//Gestion de la pagination
        ...
        const dividedPaymentList = computed(()=>{
            if(eltToSearch.value){
                return filteredpaymentList.value
                .filter((row, index) => {
        ...
        function nextPage(){
            if((currentPage.value*pageSize.value) < filteredpaymentList.value.length) currentPage.value++;
        }
        function prevPage(){
        ...
        function getPaymentRequestList(){
            axios.get(`http://${hostname}:5000/app/getPaymentRequestList`)
            .then((result) => {
                ...
                if(paymentList.value.length > 0)
                    setTimeout(()=>selectLine(paymentList.value[0]['No_']),1000)
            })
            .catch(err=>console.log(err));
        }
...
//Gestion de la sélection d'une ligne
        ...
        function displaySelectedLineCard(){
            if(selectedLine.value)
                router.push(`PaymentValidationRequestCard?documentNo=${selectedLine.value}`)
        }
        ...
        return {
            paymentList,eltToSearch,filteredpaymentList,
            dividedPaymentList,pageSize,currentPage,nextPage,prevPage,
