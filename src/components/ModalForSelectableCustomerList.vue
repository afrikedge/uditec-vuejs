    ...
    <div :class="{'modal':true , 'is-active': true,}" v-if="isActive">
        ...
        <div class="modal-card w-50 h-75">
            <header class="modal-card-head has-text-left">
                <div class="modal-card-title">
                    <div class="columns is-v-centered">
                        <div class="column is-narrow">Clients</div>
            ...
            <section class="modal-card-body" >
                ...
                <div style="margin-left: auto;margin-right: auto;" v-if="filteredCustomerList.length == 0 && res_fetching_data_occur">
                    Aucun enregistrement n'a été trouvé
                </div>
                <div v-if="res_fetching_data_occur && filteredCustomerList.length > 0 && !error_fetching_data_occur">
                    <table class="table is-narrow is-hoverable is-bordered is-fullwidth shadow tableFixHead">
                        <thead class=" my-2">
                            <tr> 
                                <th class="has-background-light has-text-grey has-text-left has-text-weight-normal is-size-5" style="min-width: 100px;">Code client</th>
                                <th class="has-background-light has-text-grey has-text-left has-text-weight-normal is-size-5" style="min-width: 100px;">Nom du client</th>
                            </tr>
                        </thead>
                        <tbody s>
                            <tr  :id="elt['No_']" v-for="(elt,index) of dividedCustomerList" :key="index" @mouseover="setLineShadow(elt['No_'])" @mouseout="removeLineShadow(elt['No_'])" @click="$emit('onGettingLineFromSelectableCustomerListModal',elt);$emit('closeModal')">
                                <td class="has-text-left is-narrow has-text-orange"> {{ elt['No_'] }} </td>
            ...
            <footer class="modal-card-foot">
                <div class="buttons">
                    ...
                    <button class="button has-background-orange has-text-white" @click="pageSize = pageSize + 50" v-if="(pageSize < filteredCustomerList.length) && (currentPage !== Math.ceil(filteredCustomerList.length/pageSize))">
                        Charger plus
                    </button>
                    <button class="button has-text-orange" @click="nextPage" v-if="currentPage*pageSize < filteredCustomerList.length">
                        <span class="icon">
...
//Gestion de la pagination
        ...
        const dividedCustomerList = computed(()=>{
            if(eltToSearch.value){
                return filteredCustomerList.value
                .filter((row, index) => {
        ...
        function nextPage(){
            if((currentPage.value*pageSize.value) < filteredCustomerList.value.length) currentPage.value++;
        }
        function prevPage(){
...
//Gestion de la récupération de la liste à afficher
        ...
        function getCustomerList(){
            axios.get(`http://${hostname}:5000/app/getCustomerListSimple`)
            .then(result => {
        ...
        return {
            ...
            dividedCustomerList,pageSize,currentPage,nextPage,prevPage,
            getCustomerList,
