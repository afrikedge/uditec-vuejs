...
export default {
    ...
    const filteredSQList = computed(()=>
        ...
        .filter((row) => new String(row['No_']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Sell-to Customer Name']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Sell-to Customer No_']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Salesperson Code']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Location Code']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Total Amount']).toLowerCase().includes(eltToSearch.value)
                || new String(row['Total Amount Including VAT']).toLowerCase().includes(eltToSearch.value)
        )
    )
