            ...
            <div class="column mt-5" id="customer-column" style="overflow-y: scroll;overflow-x: scroll;">
                <table class="table is-narrow is-hoverable">
                    ...
                    <tbody style="">
                            ...
                            <tr  :id="elt['No_']" v-for="(elt,index) of filteredVATBUSPostingGroupList" :key="index" @click="$emit('onGettingLineFromSelectableVATGroupListModal',elt);$emit('closeModal')">
...
export default{
    name:'modal-for-selectable-vat-group-list',
    components:{
        inputSearch
    },
    data(){
    ...
    setup(){
        ...
        const filteredVATBUSPostingGroupList = computed(()=>
            elementList.value
        ...
        return {
            eltToSearch,elementList,filteredVATBUSPostingGroupList
        }
    },
    props:['isActive'],
    beforeMount(){
        axios.get(`http://${this.hostname}:5000/app/getVATBUSPostingGroupList`)
        .then(result => {
